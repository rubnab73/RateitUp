                                ...
                                <div class="flex items-center justify-between p-4 border rounded-lg">
                                    <div class="flex items-center space-x-4">
                                        <div class="flex-shrink-0">
                                            <img class="h-10 w-10 rounded-full" src="https://ui-avatars.com/api/?name={{ urlencode($follower->name) }}" alt="{{ $follower->name }}">
                                        </div>
                                        <div>
                                            <h4 class="text-lg font-semibold">{{ $follower->name }}</h4>
                                            <p class="text-sm text-gray-500">Following since {{ $follower->pivot->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
